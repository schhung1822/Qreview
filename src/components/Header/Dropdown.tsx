...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-blue text-custom-sm font-medium text-foreground flex items-center gap-1.5 capitalize ${stickyMenu ? "xl:py-4" : "xl:py-6"
      ...
      {/* <!-- Dropdown Start --> */}
      <ul
        className={`dropdown bg-background border-gray-3 dark:border-dark-3 ${dropdownToggler && "flex"} ${stickyMenu
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-blue hover:bg-surface text-foreground py-[7px] px-4.5 ${pathUrl === item.path && "text-blue bg-surface"
                } `}
            >
              {item.title}
            </Link>
