...
const CategoryProducts = ({ slug }: { slug?: string }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/categories/${resolvedSlug}`, { cache: "no-store" })
  ...
  const categoryTitle = category?.name ?? "Danh mục";

  return (
    <>
      <Breadcrumb
        title={categoryTitle}
        pages={["shop", "/", categoryTitle.toLowerCase()]}
      />
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            <div className="w-full">
              ...
              {error && (
                <div className="mb-6 rounded-md border border-red-500/30 bg-red-500/10 px-4 py-3 text-red-200">
                  {error}
                </div>
              )}
              ...
              >
                {isLoading ? (
                  Array.from({ length: productStyle === "grid" ? 8 : 4 }).map(
                    (_, index) => (
                      <div
